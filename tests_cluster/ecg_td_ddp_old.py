...
os.makedirs('.' + os.sep + 'results', exist_ok=True) 
data_folder_name = '.' + os.sep + 'results' + os.sep + 'out_' +  datetime.now().strftime("%d%m%y_%H%M%S")
os.makedirs(data_folder_name, exist_ok=True) 
def ensure_directory_creation():
    try:
        local_rank = int(os.environ["LOCAL_RANK"])
    except KeyError:
        local_rank = 0  # Default to 0 if not running in distributed mode (this is for fallback)
    
    if local_rank == 0:  # Only driver (rank 0) should create the directory
        out_folder_name = '.' + os.sep + 'results' + os.sep + 'out_' + datetime.now().strftime("%d%m%y_%H%M%S")
        os.makedirs(out_folder_name, exist_ok=True)
    return out_folder_name
...
def training(models, train_datasets, criterion, num_epochs, batch_size, learning_rate, print_epoch):
    ...
    local_rank = int(os.environ["LOCAL_RANK"])
    print(f"Worker {local_rank} is processing chunk {local_rank + 1} with {len(train_datasets[local_rank])} records.")
    out_folder_name = ensure_directory_creation()
    print(f"Worker {local_rank} is processing chunk {local_rank + 1} with {len(train_datasets[local_rank])} records.")
    ...
    # torch.save(model.state_dict(), out_folder_name + os.sep + "ensemble_" + str(local_rank) + ".pth")
    model_path = os.path.join(out_folder_name, f"ensemble_{local_rank}.pth")
